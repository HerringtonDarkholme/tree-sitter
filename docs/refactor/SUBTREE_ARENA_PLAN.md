...
| 3 | Fuse deterministic-window child copying with reduction summarization (reverted: -1.19%; direct-final retries rejected) | Accumulates the parent summary while performing the already-required child-handle move, removing a second traversal and repeated index resolution | The stack accumulator regressed all seven languages; the preallocated final-parent implementation reached only +0.58% in its long gate and failed the per-language floor, and an independent retry also regressed Java | This family is closed unless a new design removes more work or materially changes the measured dependency chain |
